...
public final class Api extends com.google.api.client.json.GenericJson {
   ...
   * as in `1.10`. If the minor version is omitted, it defaults to zero. If the entire version field
   * is empty, the major version is derived from the package name, as outlined below. If the field
   * is not empty, the version in the package name will be verified to be consistent with what is
   * provided here.
   *
   ...
   *
   * The major version is also reflected in the package name of the API, which must end in `v`, as
   * in `google.feature.v1`. For major versions 0 and 1, the suffix can be omitted. Zero major
