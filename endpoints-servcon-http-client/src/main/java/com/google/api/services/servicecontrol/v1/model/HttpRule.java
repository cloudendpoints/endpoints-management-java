...
/**
 * `HttpRule` defines the mapping of an RPC method to one or more HTTP REST APIs.  The mapping
 * determines what portions of the request message are populated from the path, query parameters, or
 * body of the HTTP request.  The mapping is typically specified as an `google.api.http` annotation,
 ...
 *
 * ```proto service Messaging {   rpc GetMessage(GetMessageRequest) returns (Message) {     option
 * (google.api.http).get = "/v1/messages/{message_id}";   } } message GetMessageRequest {   string
 * message_id = 1; // mapped to the URL } message Message {   string text = 1; // content of the
 * resource } ```
 *
 ...
 *
 * ```proto message GetMessageRequest {   string message_id = 1; // mapped to the URL   int64
 * revision = 2;    // becomes a parameter } ```
 *
 * This enables a HTTP JSON to RPC mapping as below:
 *
 * HTTP | RPC -----|----- `GET /v1/messages/123456?revision=2` | `GetMessage(message_id: "123456"
 * revision: 2)`
 *
 ...
 *
 * ```proto service Messaging {   rpc UpdateMessage(UpdateMessageRequest) returns (Message) {
 * option (google.api.http) = {       put: "/v1/messages/{message_id}"       body: "message"     };
 * } } message UpdateMessageRequest {   string message_id = 1; // mapped to the URL   Message
 ...
 *
 * ```proto service Messaging {   rpc UpdateMessage(Message) returns (Message) {     option
 * (google.api.http) = {       put: "/v1/messages/{message_id}"       body: "*"     };   } } message
 * Message {   string message_id = 1;   string text = 2; } ```
 *
 ...
 *
 * ```proto service Messaging {   rpc GetMessage(GetMessageRequest) returns (Message) {     option
 * (google.api.http) = {       get: "/v1/messages/{message_id}"       additional_bindings {
 * get: "/v1/users/{user_id}/messages/{message_id}"       }     };   } } message GetMessageRequest {
 ...
 * assumes there is no HTTP body. 2. Leaf fields (recursive expansion of nested messages in the
 * request) can be classified into three types:     (a) Matched in the URL template.     (b) Covered
 * by body (if body is `*`, everything except (a) fields;         else everything under the body
 * field)     (c) All other fields. 3. URL query parameters found in the HTTP request are mapped to
 * (c) fields. 4. Any body sent with an HTTP request can contain only (b) fields.
 *
 ...
 * 6570](https://tools.ietf.org/html/rfc6570) Section 3.2.3 Reserved Expansion.
 *
 * The syntax `LITERAL` matches literal text in the URL path.
 *
 * The syntax `Variable` matches the entire path as specified by its template; this nested template
 * must not contain further variables. If a variable matches a single path segment, its template may
 * be omitted, e.g. `{var}` is equivalent to `{var=*}`.
 *
 * NOTE: the field paths in variables and in the `body` must not refer to repeated fields or map
 * fields.
 *
 * Use CustomHttpPattern to specify any HTTP method that is not included in the `pattern` field,
 * such as HEAD, or "*" to leave the HTTP method unspecified for a given URL path rule. The wild-
 * card rule is useful for services that provide content to Web (HTML) clients.
 *
...
public final class HttpRule extends com.google.api.client.json.GenericJson {
  ...
  /**
   * Do not use this. For media support, add instead [][google.bytestream.RestByteStream] as an API
